...
cartUser.addEventListener('click', () => {
  ...
  if (!cart.items) {
    showModalAlert(
      'Carrinho vazio!!',
      'Adicione itens ao carrinho para comprar.',
      closeModal
    );
    return;
  }
...
const updateCartModal = (cart) => {
  ...
  if (!cart || cart.items.length === 0) {
    ...
    showModalAlert(
      'Carrinho vazio!',
      'Todos os itens foram excluídos do carrinho.',
      closeModal
    );
    return;
  }
  ...
  document
    .querySelectorAll(".btn-stock-review[data-action='remove']")
    .forEach((button) => {
  ...
  document
    .querySelectorAll(".btn-stock-review[data-action='add']")
    .forEach((button) => {
      button.addEventListener('click', async () => {
        const productId = button.closest('tr').getAttribute('data-product-id');
  ...
  try {
    ...
    if (dataCart.items) {
      ...
      if (cart.res.ok) {
        ...
        showModalAlert(
          'Falha no carrinho!',
          'Não está buscando carrinho do usuario',
          closeModal
        );
      }
    } else {
    ...
    showModalAlert(
      'Erro de conexão',
      `Falha ao inserir item no carrinho ${error.message}`,
      closeModal
    );
  }
  await cartUserData();
