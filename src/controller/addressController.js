import { userAddress, createAddress, editAddress, deleteAddress } from "../js/service.js";
import { createOrder } from './orderController.js';
  ...
  async function showModalAddress(data, mode, type) {
    ...
    if (document.getElementById('backCheckCart')) {
      document.getElementById('backCheckCart').addEventListener('click', async () => {
        modal.style.display = "none";
      });
    };
    ...
    if (document.getElementById('nextCheckOrder')) {
      document.getElementById('nextCheckOrder').addEventListener('click', async () => {
    ...
    if (document.getElementById('deleteBtn')) {
      document.getElementById('deleteBtn').addEventListener('click', async () => {
        showModalConfirm('Confirma Excluir!!', 'Deseja excluir endereço?', async () => {
          document.getElementById('modal-address').style.display = "none";
          try {
            const data = await deleteAddress(dataAddress._id);
            if (data.res.ok) {
              showModalAlert("Endereço excluído!", data.data.message, resetModal);
            } else {
    ...
    if (document.getElementById('editBtn')) {
      document.getElementById('editBtn').addEventListener('click', async () => {
        if (postalCode.value === "") {
          showModalAlert("CEP Obrigatório", "Digite o CEP do endereço de entrega", resetModal);
          return;
        }
        if (street.textContent === "" || district.textContent === "" || city.textContent === "" || state.textContent === "") {
          showModalAlert("Endereço Obrigatório", "Digite um CEP válido para carregar o endereço de entrega", closeModal);
          return;
        }
        if (number.value === "") {
          showModalAlert("Número Obrigatório", "Digite o número do endereço de entrega", closeModal);
          return;
        }
    ...
    if (document.getElementById('createBtn')) {
      document.getElementById('createBtn').addEventListener('click', async () => {
        if (postalCode.value === "") {
          showModalAlert("CEP Obrigatório!!", "Digite o CEP do endereço de entrega", closeModal);
          return;
        }
        if (street.textContent === "" || district.textContent === "" || city.textContent === "" || state.textContent === "") {
          showModalAlert("Endereço Obrigatório!!", "Digite um CEP válido para carregar o endereço de entrega", closeModal);
        ...
        if (number.value === "" || number.value < 1) {
          showModalAlert("Número Inválido!!", "Digite um número válido para o endereço.", closeModal);
          return;
        }
        try {
        ...
        if (isValid) {
          ...
          if ((numberField.value !== '') && (numberField.value > 0)) {
            ...
            if (mode === 'create'){
              ...
              if(addressFooterCode){
                addressFooterCode.classList.remove('hidden');
                addressFooterCode.textContent = "Endereço válido para cadastrar";
              }
            }
          } else {
            if (mode === 'edit') {
    ...
    async function reqPostalCode(code) {
      try {
        const postalCodeQuery = await fetch(`https://viacep.com.br/ws/${code}/json/`);
