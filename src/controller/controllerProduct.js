import { publicProducts, sendProductData, productDelete } from "../js/service.js";
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  categoryFilter.addEventListener('change', () => {
    ...
    filteredProducts = products.filter(product => !selectedCategory || product.category === selectedCategory);

    const filteredTypes = [...new Set(filteredProducts.map(product => product.type))];
    typeFilter.innerHTML = `<option value="">Selecione um tipo</option>` + filteredTypes.map(type => `
  ...
  typeFilter.addEventListener('change', () => {
    ...
    filteredProducts = products.filter(product => (!categoryFilter.value || product.category === categoryFilter.value) && (!selectedType || product.type === selectedType));

    const filteredBrands = [...new Set(filteredProducts.map(product => product.brand))];
    brandFilter.innerHTML = `<option value="">Selecione uma marca</option>` + filteredBrands.map(brand => `
  ...
  deleteButtons.forEach(btn => {
    btn.addEventListener('click', async (e) => {
      ...
      showModalConfirm("Confirma Excluir?", "Deseja mesmo excluir esse produto?", () => {
  ...
  async function deleteProduct(productId) {
    try {
     ...
     if (response.message === "Produto e imagens excluídos com sucesso.") {
       modal.style.display = "none"
       showModalAlert("Produto Excluído", "Produto removido com sucesso!", closeModal);
       showModalListProduct();
     } else {
  ...
  async function showModalRegisterProduct() {
   ...
   `;
    ...
    function addDetail() {
      ...
      detailContainer.querySelector('.delete-btn').addEventListener('click', () => {
    ...
    function setupImageSection(section) {
      ...
      fileInput.addEventListener("change", (event) => {
        ...
        if (file) {
          ...
          if (imageIndex > -1) {
            uploadedImages[imageIndex] = file;
          } else {
    ...
    function generateImageNames(brand, model, color, totalImages) {
      const baseName = `${normalizeString(brand).replace(/\s+/g, '-')}-${normalizeString(model).replace(/\s+/g, '-')}-${normalizeString(color).replace(/\s+/g, '-')}`;
      const imageNames = [];
      for (let i = 1; i <= totalImages; i++) {
        imageNames.push(`${baseName}_${i.toString().padStart(2, '0')}`);
      }
      return imageNames;
    }
    ...
    sendProduct.addEventListener('click', () => { 
     showModalConfirm("Confirma Cadastro?", "Produto está pronto e revisado para cadastro?", handleFormSubmit)
    });
    ...
    async function handleFormSubmit() {
      ...
      const product = {
        ...
        price: price.value.replace("R$ ", "").replace(",", "."),
        stock: stock.value,
      ...
      try {
         const productData = await sendProductData(product, uploadedImages, imageNames);
         if(productData.res.ok){
           showModalAlert("Produto Cadastrado!", `Produto cadastrado com sucesso!`, closeModal);
         } else {
           showModalAlert("Não Cadastrado!!", productData.data.message, closeModal);
         }
      } catch (error) {
        showModalAlert("Erro de conexão", error.message, closeModal);
      }
