  ...
  async function showModalListOrder() {
      ...
      filter.addEventListener('change', () => {
        ...
        if (filterValue === 'delivery') {
          document
            ...
            .forEach((row) => {
              const delivery = orders.find(
                (order) => order._id === row.getAttribute('data-order-id')
              ).delivery;
    ...
    updateButtons.forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        showModalConfirm(
          'Confirma Alterar?',
          'Alterar Status do pedido para Pedido em Entrega?',
          () => {
            updateOrder(orderId);
          }
        );
    ...
    async function updateOrder(orderId) {
      try {
        ...
        if (response.data.message === 'Status alterado.') {
          ...
          showModalAlert(
            'Pedido Alterado!',
            'Status do pedido alterado para Pedido em Entrega',
            closeModal
          );
          showModalListOrder();
        } else {
