    ...
    send.addEventListener('click', async (event) => {
      ...
      try {
        ...
        if (data.res.ok) {
          showModalAlert(
            'Usuário cadastrado!',
            `Administrador ${name} cadastrado com email: ${email}.`,
            closeModal
          );
          modal.style.display = 'none';
        } else {
    ...
    send.addEventListener('click', async (event) => {
      ...
      try {
        ...
        if (data.res.ok) {
          ...
          showModalAlert(
            'Não alterado!',
            'Houve um erro ao alterar os dados. Verifique o e-mail e senha e tente novamente.',
            closeModal
          );
        }
      } catch (error) {
  ...
  async function showModalListUser() {
    ...
    filters.forEach((filter) => {
      filter.addEventListener('change', () => {
        ...
        if (filterValue === 'admin') {
          document
            .querySelectorAll('.details-table tbody tr')
            .forEach((row) => {
              const isAdmin = users.find(
                (user) => user._id === row.getAttribute('data-user-id')
              ).isAdmin;
    ...
    deleteButtons.forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        showModalConfirm(
          'Confirma Excluir?',
          'Deseja maesmo excluir esse usuário?',
          () => {
