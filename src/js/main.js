import { addToCart, removeFromCart, addToCartPurchase } from '../controller/cartController.js';
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  const displayProducts = (products) => {
    ...
    container.innerHTML = products.map(product => {
      ...
      return `
        <div class="card" data-product-id="${product._id}" data-product-name="${product.name}">
          <div class="carousel">
            ${getImageCarousel(product.images)}
          </div>
          <h3>${product.name}</h3>
          <div class="card-text">
            <p>${product.type} ${product.brand} ${product.model} ${product.color}</p>
          </div>
          <div class="detals">
            <span class="detals-price">R$ ${parseFloat(product.price).toFixed(2).replace('.', ',')}</span>
    ...
    document.querySelectorAll(".detals-btn[data-action='detals']").forEach(button => {
    ...
    document.querySelectorAll(".btn-cart[data-action='add']").forEach(button => {
    ...
    document.querySelectorAll(".btn-cart[data-action='remove']").forEach(button => {
      button.addEventListener('click', async () => {
        const productId = button.closest('.card').getAttribute('data-product-id');
        const productName = button.closest('.card').getAttribute('data-product-name');
        await removeFromCart(productId, productName);
      });
    });
  
    document.querySelectorAll(".btn-order[data-action='create-order']").forEach(button => {
      button.addEventListener('click', async () => {
        const productId = button.closest('.card').getAttribute('data-product-id');
  ...
  const getImageCarousel = (images) => {
    ...
    const imageElements = images.map((image, index) => `
      <img src="${typeof image === 'object' ? Object.values(image).join('') : image}" alt="Image ${index + 1}" class="carousel-image ${index === 0 ? 'active' : ''}">
    `).join("");
