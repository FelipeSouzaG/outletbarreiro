...
export const createAddress = async (postalCode, street, number, complement, district, city, state) => {  
 const addressData = {
...
export const editAddress = async (addressId, postalCode, street, number, complement, district, city, state) => {  
 const addressData = {
   postalCode: postalCode,
   street: street,
   number: number,
 ...
 try {
   const response = await fetch(`${apiUrl}/api/addresses/update/${addressId}`, {
     method: "PUT",
     headers: {
       "Content-Type": "application/json",
       "Authorization": `Bearer ${localStorage.getItem("token")}`
...
export const deleteAddress = async (userAddress) => {
 try {
   const response = await fetch(`${apiUrl}/api/addresses/delete/${userAddress}`, {
     method: "DELETE",
     headers: {
       "Authorization": `Bearer ${localStorage.getItem("token")}`
     }
...
try {
  ...
  const response = await fetch(`${apiUrl}/api/orders/update/${orderId}/status`, {
    method: "PUT",
    headers: {
      "Content-Type": "application/json",
      "Authorization": `Bearer ${localStorage.getItem("token")}`
...
export const sendProductData = async (product, uploadedImages, imageNames) => {
 ...
 try {
   ...
   throw new Error('Erro ao tentar registrar produto', error);
 }
};
